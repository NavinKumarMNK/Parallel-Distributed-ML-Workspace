...
def run_script(ip, username, password):
    os.system(f'sshpass -p {password} scp -o StrictHostKeyChecking=no ray.sh {username}@{ip}:~/')
    os.system(f'sshpass -p {password} ssh -o StrictHostKeyChecking=no {username}@{ip} "chmod +x ~/ray.sh && ~/ray.sh"')
